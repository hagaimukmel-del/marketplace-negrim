    ...
    >
      <div className="flex max-h-[94dvh] w-full flex-col overflow-hidden rounded-t-2xl bg-white sm:max-w-lg sm:rounded-2xl">
        <div className="relative">
          <div className="flex snap-x snap-mandatory overflow-x-auto bg-stone-900" dir="ltr">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          <div>
            ...
            <h2 className="mt-1 text-xl font-bold leading-tight text-stone-900">{card.title}</h2>
            <div className="mt-2 flex flex-wrap items-baseline gap-x-3 gap-y-1">
              {card.dealType === 'free' ? (
                <span className="flex items-center gap-1.5 text-lg font-bold text-emerald-800">
                  <Gift size={18} />
                  למסירה בחינם
                </span>
              ) : (
                <>
                  <span className="tnum text-2xl font-bold text-stone-900">{formatIls(card.pricePerUnit!)}</span>
                  <span className="text-sm text-stone-500">ל{card.unit}</span>
                </>
              )}
              <span className="tnum text-sm text-stone-600">
          ...
          {card.description && <p className="whitespace-pre-line text-stone-700">{card.description}</p>}
          ...
          {card.mine ? (
            <p className="rounded-lg bg-stone-100 p-3 text-sm text-stone-600">זו המודעה שלך. ניהול ועריכה — ב״המודעות שלי״.</p>
          ) : contact ? (
            <div className="rounded-xl bg-emerald-50 p-3">
              <p className="text-sm text-emerald-900">{contact.name}</p>
              <p dir="ltr" className="tnum text-end text-2xl font-bold text-stone-900">{formatPhone(contact.phone)}</p>
              <div className="mt-3 grid grid-cols-2 gap-2">
                <a
                  href={whatsappUrl(contact.phone, message)}
                  target="_blank"
                  rel="noreferrer"
                  className="flex h-12 items-center justify-center gap-2 rounded-lg bg-emerald-700 font-bold text-white"
                >
                  <MessageCircle size={18} />
                  וואטסאפ
                </a>
                <a href={`tel:${contact.phone}`} className="flex h-12 items-center justify-center gap-2 rounded-lg border border-emerald-700 bg-white font-bold text-emerald-800">
                  <Phone size={18} />
                  חיוג
                </a>
              </div>
            ...
            <div>
              <button
                ...
                onClick={reveal}
                disabled={loading}
                className="flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-emerald-700 font-bold text-white disabled:opacity-60"
              >
                <Phone size={18} />
          ...
          {!card.mine && (
            <div className="border-t border-stone-100 pt-3">
              {reported ? (
                <p className="text-sm text-stone-500">תודה — הדיווח הועבר לבדיקה.</p>
              ) : reporting ? (
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-stone-800">מה הבעיה במודעה?</p>
                    ...
                    {METZION_REPORT_REASONS.map((item) => (
                      <button
                        ...
                        className={`h-9 rounded-full border px-3 text-sm ${
                          reason === item.key ? 'border-stone-900 bg-stone-900 text-white' : 'border-stone-300 text-stone-700'
                        }`}
                      >
                        {item.label}
                      </button>
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={report} disabled={!reason} className="h-10 rounded-lg bg-stone-900 px-4 text-sm font-semibold text-white disabled:opacity-40">
                      שלח דיווח
                    </button>
                    <button type="button" onClick={() => setReporting(false)} className="h-10 rounded-lg px-3 text-sm text-stone-600">
                      ביטול
                    </button>
                  </div>
                </div>
              ) : (
                <button type="button" onClick={() => setReporting(true)} className="flex items-center gap-1.5 text-sm text-stone-500 hover:text-stone-800">
                  <Flag size={14} />
                  דווח על המודעה
                </button>
              )}
